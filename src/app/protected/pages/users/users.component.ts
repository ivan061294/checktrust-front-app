...
export class UsersComponent implements OnInit {
  ...
  linkNext: string = '';
  linkPrev: string = '';
  usernamekey = new BehaviorSubject<string | null>(null);
  ...
  buscarPorNombreUsuarios(): void {
    this.buscarUsuariosDelSistema('', this.usernamekey.value?.trim());
  }
  ...
  validarUserKey(event: any) {
    this.usernamekey.next(event.target.value);
  }
  ...
  buscarUsuariosDelSistema(href: string, username: string | null = null) {
    if (href.length === 0) {
      href = '/api/v1/users';
    }
    ...
    this.protecService.buscarUsuariosDelSistema(href, username).subscribe(
      (users) => {
        this.users = users;
  ...
  onNewUser() {
    ...
    this.miFormulario.get('dni')?.setValue('');
    this.miFormulario.get('claveac')?.setValue('');
  ...
  onEditUser(user_id: number) {
    ...
    this.protecService.buscarUsuario(user_id).subscribe(
      (user) => {
        this.userData = user;
        this.loadDataUser = true;
  ...
  onSave() {
    ...
    if (this.newDataUser) {
      const { fullname, nameuser, claveac, profile, status, dni } =
      ...
      const { fullname, nameuser, claveac, profile, status, dni } =
        this.miFormulario.value;
      const body =
        claveac !== this.paswAleatorio
          ? { password: claveac, profile, status, dni: dni }
          : { profile, status, dni };
