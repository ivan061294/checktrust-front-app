...
import {
  IListRestrictive,
  ISenInfoFinResponse,
} from '../interfaces/info-financiera';
...
export class ProtectedService {
  ...
  buscarInfoFinancieraPorDNI_V2(
    tipodoc: string,
    dni: string
  ): Observable<ISenInfoFinResponse> {
  ...
  buscarInfoFinanciera_DocRelacionado(
    href: string
  ): Observable<ISenInfoFinResponse> {
  ...
  buscarUsuariosDelSistema(
    href: string,
    username: string | null
  ): Observable<IUsers> {
    const url = `${this.baseUrl}${href}`;
    const autorization = 'Bearer ' + sessionStorage.getItem('token') || '';
    const params = new HttpParams().set('fullname', username || '');

    const headers = new HttpHeaders().set('Authorization', autorization);

    return this.http.get<IUsers>(url, { headers: headers, params: params });
  }
  buscarPeticionesProveddores(href: string): Observable<any> {
    const url = `${this.baseUrl}${href}`;
    const autorization = 'Bearer ' + sessionStorage.getItem('token') || '';
